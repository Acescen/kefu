<template>
  <div class="app-container">
    ...
    <div class="filter-container">
      <el-input v-model="listQuery.username" placeholder="请输入账号搜索" style="width: 200px;" class="filter-item" @keyup.enter.native="handleFilter" />
    ...
    >
      ...
      <el-table-column label="账号" min-width="160px">
        <template slot-scope="{row}">
...
<!--      </el-table-column>-->
      ...
      <el-table-column label="操作" align="center" min-width="230px" class-name="small-padding fixed-width">
        <template slot-scope="{row}">
          ...
          <el-button v-if="row.status != '2'" size="mini" type="danger" @click="deleteRow(row,'2')">
            删除
          </el-button>
...
export default {
  ...
  data() {
    return {
      ...
      level: 10, // 当前用户级别，随便写的，暂时没用上。做数据权限
      allRoles: [], // 所有角色集合
      ...
      rules: {
        nickname: [
          ...
          { min: 1, max: 10, message: '账号长度在 1 到 10 个字符', trigger: 'blur' }
        ],
        username: [
          { required: true, message: '请输入账号', trigger: 'blur' },
          { min: 5, max: 18, message: '账号长度在 5 到 18 个字符', trigger: 'blur' }
        ],
        password: [
