    ...
    <div class="app-container" :style="'height:' + (clientHeight - 50) + 'px;'">
            ...
            >
                <el-table-column label="会话列表">
                    <template slot-scope="{row}">
                        <span>{{ getContact(row).nickname +' ('+ getContact(row).username +')'}}</span>
                    </template>
                </el-table-column>
            </el-table>
        </div>
        ...
        <div class="center-container">
            <div v-if="conversation" class="chat-container">
                <!-- 导航栏 -->
                <div class="chat-nav">
                    <span v-text="contact.nickname +' ('+ contact.username+')' "/>
                </div>
                            ...
                            <ul>
                                ...
                                >
                                    ...
                                    <div v-else :class="'main' + (isOneself(message) ? ' self' : '')">
                                        ...
                                        <img
                                                class="avatar"
                                                :src="isOneself(message) ? getImgUrl(user.avatar) : getImgUrl(contact.avatar)"
                                                alt="头像图片"
                                        >
    ...
    export default {
        data() {
            return {
                ...
                user: this.$store.getters.userInfo // 当前用户对象
            }
        },
        // 不能操作DOM
        created() {
        ...
        methods: {
            ...
            getContact(conversation) {
                ...
                if (this.user.id === conversation.fromUserId) {
                    // console.log(conversation.toUser)
                    return conversation.toUser
                } else {
                    // console.log(conversation.fromUser)
